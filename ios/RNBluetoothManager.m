...
NSString *EVENT_BLUETOOTH_STATE=@"EVENT_BLUETOOTH_STATE";
...
- (NSDictionary *)constantsToExport
{
    ...
    /*
     ...
     EVENT_BLUETOOTH_STATE Emits when bluetooth state update
     */
    ...
    return @{ EVENT_DEVICE_ALREADY_PAIRED: EVENT_DEVICE_ALREADY_PAIRED,
              ...
              EVENT_BLUETOOTH_STATE:EVENT_BLUETOOTH_STATE};
}
...
-(void)callStop{
    if(self.centralManager.isScanning){
        ...
        if(hasListeners){
            [self sendEventWithName:EVENT_DEVICE_DISCOVER_DONE body:@{@"found":devices}];
        }
        if(self.scanResolveBlock){
            self.scanResolveBlock(@{@"found":devices});
...
{
    NSLog(@"(@synchronized) _centralManager");
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central{
    ...
    stateNumber = [NSNumber numberWithLong:central.state];
    
    if(hasListeners){
        [self sendEventWithName:EVENT_BLUETOOTH_STATE body:@{@"state":stateNumber}];
    }
}
...
- (void)centralManager:(CBCentralManager *)central didFailToConnectPeripheral:(CBPeripheral *)peripheral error:(nullable NSError *)error{
    if(self.connectRejectBlock){
        ...
        self.connectRejectBlock(errorCode,error.localizedFailureReason,error);
        self.connectRejectBlock = nil;
        self.connectResolveBlock = nil;
        _waitingConnect = nil;
    }
